...
public class VerleihServiceImplTest
{
    ...
    @Test
    public void testMesterkeVor() throws Exception
    {
        Medium vormerkmedium = _medienListe.get(0);
        _service.merkeVor(_vormerkkunde, vormerkmedium);
        assertEquals(_service.getVormerkerKarteFuer(vormerkmedium)
            .getVormerker(0), _vormerkkunde);

    }

    @Test
    public void testgetVormerkerkarten()
    {
        Medium vormerkmedium = _medienListe.get(0);
        _service.merkeVor(_kunde, vormerkmedium);
        VormerkerKarte erstesElement = _service
            .getVormerkerKarteFuer(vormerkmedium);
        VormerkerKarte ersteVormerkerkarte = _service.getVormerkerkarten()
            .get(0);

        assertEquals(erstesElement, ersteVormerkerkarte);
    }

    @Test
    public void testGetVormerkerKarteFuer()
    {
        Medium _medium = _medienListe.get(0);
        _service.merkeVor(_kunde, _medium);
        assertEquals(_service.getVormerkerkarten()
            .get(0), _service.getVormerkerKarteFuer(_medium));
    }

    @Test
    public void testHatVormerkerKarte()
    {
        Medium _medium = _medienListe.get(0);
        _service.merkeVor(_kunde, _medium);
        assertTrue(_service.hatVormerkerKarte(_medium));
    }

    @Test
    public void testeVerleihUndRueckgabeVonMedien() throws Exception
    {
        // Lege eine Liste mit nur verliehenen und eine Liste mit ausschlieÃŸlich
        ...
        // PrÃ¼fe, ob alle sondierenden Operationen fÃ¼r das Vertragsmodell
        // funktionieren
        ...
        // PrÃ¼fe alle sonstigen sondierenden Methoden
