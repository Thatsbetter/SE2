 ...
 * mit ihren fÃ¼r das Vormerken relevanten Verleihinformationen.
 * 
...
public class VormerkMedienauflisterWerkzeug extends ObservableSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        {
            // TODO fÃ¼r Aufgabenblatt 6 (nicht lÃ¶schen): Die
            // VormerkMedienFormatierer mÃ¼ssen noch mit einem mÃ¶glichen
            // Entleiher und mÃ¶glichen Vormerkern ausgestattet werden.
            // Ist dies korrekt implementiert, erscheinen in der Vormerkansicht
            // die Namen des Entleihers und der mÃ¶glichen 3 Vormerker.
            ...
            {
                VormerkerKarte _vormerkKarte = _verleihService
                    .getVormerkerKarteFuer(medium);
                for (int k = 0; k < _vormerkKarte
                    .getAktuelleListenLaenge(); k++)
                {
                    if (k == 0)
                    {
                        if (_vormerkKarte.getVormerker(0) != null)
                        {
                            vormerker1 = _vormerkKarte.getVormerker(0);
                        }

                    }
                    else if (k == 1)
                    {
                        if (_vormerkKarte.getVormerker(1) != null)
                        {
                            vormerker2 = _vormerkKarte.getVormerker(1);
                        }
                    }
                    else
                    {
                        if (_vormerkKarte.getVormerker(2) != null)
                            vormerker3 = _vormerkKarte.getVormerker(2);
                    }
                    // vormerker1 = _vormerkKarte.getVormerker(0);
                    //vormerker2 = _vormerkKarte.getVormerker(1);
                    //vormerker3 = _vormerkKarte.getVormerker(2);
