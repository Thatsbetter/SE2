        ...
        implements VerleihService
{
    ...
    private Map<Medium, VormerkerKarte> _vormerkerkarten;
    ...
    /**
     * Der Protokollierer fÃ¼r die VerleihvorgÃ¤nge.
     */
            ...
            List<Verleihkarte> initialBestand)
    {
        ...
        _vormerkerkarten = new HashMap<Medium, VormerkerKarte>();
    ...
    {
        return new ArrayList<VormerkerKarte>(_vormerkerkarten.values());
    }
    ...
    {
        if (hatVormerkerKarte(medium))
        {
            return _vormerkerkarten.get(medium)
                .getAktuelleListenLaenge() < VormerkerKarte.MAXVORMERKER;
        }
        else
        {
            return true;
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            ...
            if (_vormerkerkarten.containsKey(medium)
                    && _vormerkerkarten.get(medium)
                        .getAktuelleListenLaenge() > 0
                    && kunde == _vormerkerkarten.get(medium)
                        .getVormerker(0))
                if (hatVormerkerKarte(medium))
            {
                _vormerkerkarten.get(medium)
    ...
    public void merkeVor(Kunde kunde, Medium medium)
    {
        ...
        {
            _vormerkerkarten.put(medium, new VormerkerKarte(kunde, medium));
        }
        informiereUeberAenderung();
    }
    ...
    @Override
    public VormerkerKarte getVormerkerKarteFuer(Medium medium)
    {
        assert hatVormerkerKarte(
                medium) : "Vorbedingung verletzt: es existiert keine VormerkerKarte";
        return _vormerkerkarten.get(medium);
    }
    ...
    @Override
    public boolean hatVormerkerKarte(Medium medium)
    {
        assert mediumImBestand(medium);
        return _vormerkerkarten.containsKey(medium);
